...
## 18. Standar UI/UX Modal & Skala Intensitas Surveillance Heatmap

### A. Panduan Rumus Skala Intensitas (Scale Intensity)
Pada section *Surveillance Location Summary Heatmap*, disediakan tombol **Info `(i)`** di samping teks `Scale Intensity:` yang membuka modal panduan rumus penentuan skala:
...
1. **Rumus Normalisasi Relatif Dinamis**:
   ...
   - Menggunakan nilai tertinggi dalam tahun berjalan se-ASEAN ($V_{\text{max}}$) sebagai penyebut agar sensitivitas matriks visual selalu terjaga terhadap dinamika musiman.
2. **Kriteria & Ambang Batas 5 Tingkatan**:
   - **Zero (0)**: $\text{Value} = 0$ ($R = 0\%$) — Nihil / tidak ada laporan kasus maupun kematian yang terdeteksi.
   - **Low (Hijau)**: $0 < R < 5\%$ — Insidensi dasar / penularan sporadis terkendali.
   - **Med (Amber)**: $5\% \le R < 20\%$ — Transmisi sedang / kluster aktif berkelanjutan.
   - **High (Oranye)**: $20\% \le R < 55\%$ — **Lonjakan signifikan (*Significant Outbreak Elevation*)**, kasus meningkat tajam di atas rata-rata tren historis tahunan dan memerlukan intervensi dini.
   - **Critical / Peak (Merah)**: $R \ge 55\%$ — **Puncak epidemi (*Epidemic Peak*)**, beban penyakit tertinggi tahun berjalan (fase kritis).
3. **Metrik Epidemiologi**:
   - **Detected Cases**: Total kasus terdeteksi hasil ekstraksi model NLP.
   - **Recorded Deaths**: Total kematian terkonfirmasi dari artikel terpublikasi.
   - **Case Fatality Rate (CFR %)**: $\text{CFR (\%)} = \left( \frac{\text{Deaths}}{\text{Cases}} \right) \times 100\%$.

### B. Standar UI/UX Modal (Wide Layout & Clean Titles)
Untuk memastikan antarmuka terasa premium, nyaman diakses, dan bebas dari isu visual:
1. **Modal Berformat Lebar (*Wide Layout*)**: Seluruh modal informasi (`CrawlingInfoModal`, `KpiInfoModal`, `ScaleIntensityModal`, dan `selectedCountryModal`) menggunakan ukuran `max-w-3xl sm:max-w-4xl` sehingga tabel rincian sumber, metrik, dan grafik dapat bernapas lega tanpa scroll horizontal/vertikal sempit.
2. **Judul Bersih (*No Icons Beside Header Titles*)**: Seluruh header modal menggunakan judul teks murni tanpa penempatan ikon/avatar di sampingnya (`setiap judul sampingnya jgn ada icon`).
3. **Bebas Flickering / Blip-blip**: Menghilangkan animasi transform/zoom agresif (`zoom-in-95`) yang menyebabkan *layout shift* atau flickering saat scrollbar muncul, menggunakan transisi fade-in stabil dengan `flex-1 overflow-y-auto`.

---

## 19. NLP TV Dashboard: Location-based Surveillance Signals & Auto-Layouting System

Halaman TV Dashboard (`/tv`) dirancang untuk pemantauan layar lebar (*Command Center / Situational Room*). Bagian ini mencakup pembaruan fungsional dan tata letak dinamis:

### A. Tab Menu Location-based Surveillance Signals pada Panel Analytics & Situation
Panel samping kanan **ANALYTICS & SITUATION** kini dilengkapi dengan 4 mode tampilan tersegmentasi:
1. **All**: Menampilkan ringkasan gabungan (Kasus per Penyakit, Distribusi Regional, serta quick banner sinyal surveilans).
2. **Diseases**: Grafik batang vertikal kasus per penyakit teratas di kawasan ASEAN.
3. **Regions**: Pemetaan distribusi agregasi kasus per negara ASEAN.
4. **Signals (Location-based Surveillance Signals)**: Menu khusus yang menampilkan feed sinyal peringatan dini epidemiologi berbasis lokasi (*outbreak alerts*).

#### Fitur & Informasi pada Tab Signals:
- **Filter Severity Terintegrasi**: Tombol cepat untuk memfilter tingkatan kewaspadaan:
  - `ALL`: Seluruh sinyal yang terdeteksi.
  - `CRITICAL` (*AWAS*): Sinyal merah kritis dengan lonjakan kasus ekstrem.
  - `WARNING` (*SIAGA*): Peringatan dini lonjakan kasus sedang hingga tinggi.
  - `ADVISORY` (*WASPADA*): Peringatan awal observasi kluster kasus.
- **Pencarian Real-Time**: Kolom pencarian untuk menyaring sinyal berdasarkan nama penyakit atau nama kota/negara.
- **Struktur Kartu Sinyal**:
  - *Header Kartu*: Nama penyakit (diterjemahkan sesuai bahasa aktif) + Pill badge tingkat keparahan (*AWAS/SIAGA/WASPADA/NORMAL*).
  - *Geografi*: Bendera negara + Nama lokasi administratif dan negara.
  - *Metrik Utama*: Kasus terdeteksi (badge biru tebal) + Kematian (badge merah) + Tanggal publikasi artikel asli.
  - *Detail Akordeon Interaktif*: Klik kartu membuka detail intelijen surveilans (sumber berita/laporan, tag gejala yang diekstrak oleh NLP, kutipan teks berita, serta tombol tautan langsung ke artikel asli).

### B. Arsitektur Auto-Layouting & Hirarki Z-Index Bebas Tabrakan
Untuk mengatasi masalah kartu melayang yang saling tumpang tindih (*saling timpa*) pada berbagai resolusi layar (khususnya laptop 1080p, TV 4K, hingga layar rasio 16:10):
1. **Dynamic Top Offset Measurement (`ResizeObserver`)**:
   - Komponen induk `/tv` mengamati batas bawah aktual dari floating header dan KPI cards bar secara dinamis menggunakan `ResizeObserver`.
   - Jarak atas (*top offset*) untuk panel kiri (`CrawlingFeedPanel`) dan kanan (`AnalyticsSituationPanel`) dihitung otomatis (`Math.max(rect.bottom + 10, 74)`), sehingga tidak lagi menggunakan nilai statis `top-[198px]`.
   - Jika KPI cards membungkus menjadi 2 baris pada layar sempit, panel secara otomatis turun tanpa bertabrakan dengan kartu KPI.
2. **Animasi Smooth (*Fluid Height & Opacity*)**:
   - Menekan tombol **Hide KPI / Show KPI** tidak lagi memotong elemen secara instan, melainkan menganimasikan `max-h`, `opacity`, dan `translate-y` dengan kurva `duration-350 ease-in-out`.
   - Panel samping kiri dan kanan meluncur naik-turun secara mulus dan serempak mengikuti kartu KPI.
3. **Hover Auto-Elevation**:
   - Panel samping memiliki kelas `z-30 hover:z-45 focus-within:z-45 transition-all duration-300`, sehingga panel yang sedang difokuskan atau di-hover pengguna otomatis naik ke lapisan teratas tanpa tertutup oleh elemen peta atau popup.
   - Kartu popup marker peta (`AseanMap`) menggunakan `z-35 hover:z-50`, menjamin informasi lokasi dapat diakses dengan mudah tanpa tertutup panel samping.
