...
| **BUG-09** | **Crawling & Ekstraksi Gagal/Timeout pada Portal Berita RS (`rspp.co.id`), Judul Terpotong, Tanggal Meleset, dan Kebocoran Stopword Lokasi "Sarang"** | `services/collector-python/app/collectors/web_scraper.py`<br>`services/nlp-python/app/extractors.py` | 1. Scraper memicu stealth challenge solver pada HTTP 200 normal sehingga terjadi hanging timeout.<br>2. `og:title` di CMS terpotong sehingga ekstraktor judul mengabaikan tag `<h1>` yang lengkap.<br>3. Format tanggal `26-May-2025` di `.post-meta` tidak tertangkap regex tanggal publikasi.<br>4. Frasa entomologis "sarang nyamuk" terdeteksi sebagai lokasi wilayah ("Sarang", Jateng). | 1. Modifikasi `_fetch` di `web_scraper.py` agar tidak memicu stealth jika status HTTP 200 dan konten valid.<br>2. Prioritaskan `h1` / `h1.post-title` dibanding `og:title` jika `og:title` lebih pendek.<br>3. Tambahkan parser regex format `DD-Mon-YYYY` (May/Mei) pada metadata container.<br>4. Tambahkan `"sarang"`, `"nyamuk"`, `"jentik"`, `"psn"` ke `AMBIGUOUS_STOPWORDS`. |
| **BUG-10** | **Deskripsi / Konten Berita Tidak Tampil di Header Utama & Kartu Tanggal Publikasi Belum Ada di Grid Metrik (`/analyze`)** | `services/frontend-next/app/analyze/page.tsx`<br>`services/frontend-next/locales/id.json`<br>`services/frontend-next/locales/en.json` | 1. Teks konten/ringkasan artikel (`result.content`) tersembunyi di dalam accordion paling bawah dan tidak muncul di kartu detail utama.<br>2. Grid metrik analisis belum memiliki kartu terpisah untuk 'Tanggal Publikasi' (hanya muncul di sub-header kecil). | 1. Tampilkan cuplikan deskripsi/ringkasan berita (`result.content`) langsung di dalam kartu artikel utama di bawah judul.<br>2. Tambahkan `AnalyzeResultCard` khusus untuk **Tanggal Publikasi** (`published_at`) di grid metrik lengkap dengan ikon kalender dan tooltip sumber.<br>3. Tambahkan key i18n `publishedDate` dan `articleDescription` di `id.json` dan `en.json`. |
