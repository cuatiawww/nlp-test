          ...
          <div className="flex items-center gap-2.5 rounded-2xl bg-emerald-50/70 border border-emerald-200 p-3 text-[11px] text-slate-700 shadow-2xs">
            ...
            <div className="leading-tight">
              <p className="font-black text-slate-900">
                {t("crawling.livePipeline")}: {runningTotalCrawled.toLocaleString()} {t("crawling.sourceRecords")}
              </p>
              <p className="text-[10.5px] text-slate-500 mt-0.5">
                {(crawlingStats?.total_processed ?? 2280).toLocaleString()} {t("crawling.processedRecords")} ({(((crawlingStats?.total_processed ?? 2280) / (runningTotalCrawled || 1)) * 100).toFixed(1)}% {t("crawling.throughput")}) • {t("crawling.syncedKpi")}
              </p>
            </div>
          </div>
        </div>
