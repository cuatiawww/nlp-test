...
export default function ExecutiveDashboardPage() {
  ...
  // Executive Filter Parameters (ALL = Regional ASEAN 11, or individual country name)
  ...
  // Load All Executive Surveillance Data
  const loadExecutiveData = useCallback(async (isSilent = false) => {
    ...
    try {
      ...
      setError(err?.message || 'Failed to refresh executive surveillance data')
      toast.error('Failed to refresh executive surveillance data')
    } finally {
      setLoading(false)
      setRefreshing(false)
    }
  ...
  const handleCopyBriefing = () => {
    if (!data) return
    const text = `*EXECUTIVE DISEASE SURVEILLANCE BRIEFING*
Date of Report: ${new Date().toLocaleDateString(numLocale, { dateStyle: 'full' })}
Surveillance Scope: ${selectedCountry === 'ALL' ? 'Regional ASEAN 11' : selectedCountry} (Year ${selectedYear})
...
Source: Disease Surveillance AI Platform — ASEAN Health Intelligence.`
  ...
  if (!mounted || (loading && !data)) {
    return (
      <div className="flex min-h-[460px] w-full flex-col items-center justify-center p-12 text-sm text-slate-500">
        ...
        <p className="mt-1 text-xs text-slate-400">Loading executive surveillance intelligence...</p>
      </div>
    )
  }
  ...
  return (
    <div className="w-full space-y-6 bg-[#f8fafc] px-4 py-6 sm:px-6 lg:px-8">
      {/* ─────────────────────────────────────────────────────────────
          1. EXECUTIVE HEADER SECTION (Consistent with Home / page.tsx)
          ───────────────────────────────────────────────────────────── */}
      <section className="flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-600">
            Macro Epidemiological Intelligence, Cross-Border Threat Assessment & Strategic Directives
          </p>
        </div>
      ...
      {/* ─────────────────────────────────────────────────────────────
          2. EXECUTIVE SITUATIONAL SUMMARY & STRATEGIC DIRECTIVES
          ───────────────────────────────────────────────────────────── */}
      <section
      ...
      >
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <h2 className="text-xs font-black uppercase tracking-widest text-[#0060A9]">
                  Executive Situational Intelligence Summary
                </h2>
          ...
          {/* Right: Strategic Leadership Directives Card */}
          <div className="lg:col-span-5 rounded-xl border border-[#cfe0f1] bg-white p-4 shadow-xs space-y-2.5">
            ...
            <ul className="space-y-2 text-xs text-slate-600">
              <li className="flex items-start gap-2">
                ...
                <span>
                  <strong className="text-slate-800">Field Verification:</strong> Dispatch
                  Regional Rapid Response Teams (TGC) for clinical laboratory verification in
              ...
              <li className="flex items-start gap-2">
                ...
                <span>
                  <strong className="text-slate-800">Supply Readiness:</strong> Pre-position
                  diagnostic RDT kits, vector control supplies, and hydration fluids at primary
                  health centers in surge areas.
                </span>
              </li>
              <li className="flex items-start gap-2">
                <span className="mt-1 h-2 w-2 rounded-full bg-[#0060A9] shrink-0" />
                <span>
                  <strong className="text-slate-800">Cross-Border Screening:</strong> Heighten
                  entry point screening at international airports and seaports regarding emerging
                  regional clusters.
                </span>
              </li>
            </ul>
      ...
      {/* ─────────────────────────────────────────────────────────────
          3. 5 MACRO STRATEGIC KPI CARDS (Standard Home Page Radius & Shadow)
          ───────────────────────────────────────────────────────────── */}
            ...
            <div className="mt-3 pt-3 border-t border-blue-200/60">
              ...
              <div className="space-y-1.5 max-h-[160px] overflow-y-auto pr-1 text-xs">
                {prioritizedTriageList.length === 0 ? (
                  ...
                  prioritizedTriageList.slice(0, 3).map((loc, i) => (
                    <button
