...
export default function LocationsPage() {
  ...
  const { data: locations, loading, page, setPage, total, totalPages, search, setSearch, nextPage, prevPage, reload } = usePaginatedFetch<Location>('/api/v1/locations')
  ...
  return (
    <div className="px-4 md:px-6">
      ...
      <div className="mt-4 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        {loading ? <div className="p-8 text-center text-slate-400 text-sm">{t('common.loading')}</div> : locations.length === 0 ? (
            ...
            <tbody>
              {locations.map(l => (
                <tr key={l.id} className="border-b border-slate-50 hover:bg-blue-50/40">
                  ...
                  <td className="px-4 py-3 text-slate-700">{l.country || '-'}</td>
      ...
      <Modal open={showModal} title={editItem ? t('pages.locations.editTitle') : t('pages.locations.addTitle')} onClose={() => setShowModal(false)}>
        <LocationForm location={editItem} onSaved={() => { setShowModal(false); reload() }} onCancel={() => setShowModal(false)} />
      </Modal>
    </div>
  )
}
