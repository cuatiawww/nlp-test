    ...
    <div className="px-4 md:px-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">{t('pages.outbreakRules.subtitle')}</p>
        </div>
      ...
      <div className="mt-4 flex gap-2">
        <div className="relative max-w-xs">
          ...
          <SearchInput value={search} onChange={setSearch} placeholder={t('pages.outbreakRules.searchPlaceholder')} />
        </div>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b bg-slate-50 text-left">
                ...
                <th className="px-4 py-3 text-right font-semibold text-slate-600">{t('pages.outbreakRules.colMinCases')}</th>
                <th className="px-4 py-3 text-center font-semibold text-slate-600">{t('common.active')}</th>
                <th className="px-4 py-3 text-center font-semibold text-slate-600">{t('pages.outbreakRules.colPriority')}</th>
            ...
            <tbody>
              {rules.map(r => (
                <tr key={r.id} className="border-b border-slate-50 hover:bg-teal-50/40">
                  <td className="px-4 py-3 font-medium text-slate-800">{translateDisease(r.disease_name) || r.disease_name}</td>
      ...
      <Modal open={showModal} title={editItem ? t('pages.outbreakRules.editTitle') : t('pages.outbreakRules.addTitle')} onClose={() => setShowModal(false)}>
