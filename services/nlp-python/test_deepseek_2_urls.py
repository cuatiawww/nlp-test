...
# Ensure nlp-python app directory is in sys.path
sys.path.insert(0, "/home/aspire_5/app/NLP-PENYAKIT/services/nlp-python")

from app import config, pipeline, llm_gate
...
def test_article(url_num, title, text, source_url):
    print(f"\n--- TESTING ARTICLE {url_num}: {title[:60]}... ---")
    req = AnalyzeRequest(
        text=text,
        title=title,
    ...
    try:
        ...
        print(f"Primary Disease: {res.disease}")
        print(f"Primary Location: {res.location} (Country: {res.country})")
        print(f"Case Count: {res.case_count}, Death Count: {res.death_count}")
        print(f"Overall Confidence: {res.confidence}")
        print(f"Event Confidence: {res.event_confidence}")
        print(f"Relevance: {res.relevance} (Confidence: {res.relevance_confidence})")
