...
# The database stores names and coordinates, but does not currently expose an
# administrative-level field. Keep this conservative set for province output;
# a city or an arbitrary gazetteer row must not be promoted to a province.
INDONESIAN_PROVINCES = frozenset({
    "Aceh", "Sumatera Utara", "Sumatera Barat", "Riau", "Kepulauan Riau",
    "Jambi", "Sumatera Selatan", "Kepulauan Bangka Belitung", "Bengkulu",
    "Lampung", "DKI Jakarta", "Jawa Barat", "Banten", "Jawa Tengah",
    "DI Yogyakarta", "Daerah Istimewa Yogyakarta", "Jawa Timur", "Bali",
...
def _is_known_province(name: str, country: str) -> bool:
    return country.casefold() == "indonesia" and any(
        extractors._fold_location_text(name) == extractors._fold_location_text(province)
    ...
    def link(self, value: str, context: str = "", evidence: str = "") -> Optional[LinkedLocation]:
        ...
        if canonical:
            ...
            return LinkedLocation(
                ...
                is_province=_is_known_province(canonical, country),
                evidence=evidence,
            )
        ...
        return LinkedLocation(
            ...
            is_province=_is_known_province(value, str(result["country"])),
            evidence=evidence,
        )
...
def aggregate_relation_totals(relations: list[MetricRelation]) -> tuple[int, int]:
...
def extract_time_frame(text: str, published_date: Optional[str] = None) -> str:
    ...
    # ISO dates and day-month-year ranges, including Indonesian month names.
    iso = re.search(r"(\d{4}-\d{2}-\d{2})\s*(?:to|sampai|hingga|-|–)\s*(\d{4}-\d{2}-\d{2})", value, re.I)
    if iso:
        return f"{iso.group(1)} to {iso.group(2)}"
    named = re.search(
        r"(\d{1,2})\s+([A-Za-z]+)\s+(\d{4})\s*(?:to|sampai|hingga|-|–)\s*"
        r"(\d{1,2})\s+([A-Za-z]+)\s+(\d{4})",
        value, re.I,
    )
    if named:
        start = _date_from_parts(named.group(1), named.group(2), named.group(3))
        end = _date_from_parts(named.group(4), named.group(5), named.group(6))
        if start and end:
...
def extract_metric_relations(
    ...
    for pattern in patterns:
        for match in pattern.finditer(text or ""):
            ...
            frame = _relation_time_frame(text, linked, published_date)
            key = (linked.country.casefold(), linked.name.casefold(), frame)
            current = relations.get(key)
            if not current or count > current.cases:
                relations[key] = MetricRelation(
    ...
    for pattern in (_DEATH_NEAR_LOCATION, _DEATH_WITH_LABEL, _LOCATION_BEFORE_DEATH):
        for match in pattern.finditer(text or ""):
            ...
            if current:
                current.deaths = max(current.deaths or 0, death_count)
            else:
                relations[key] = MetricRelation(
...
def build_surveillance_output(
    ...
    *,
    ...
    alert, relevance, signal = decide_alert(text, relations, labels)
    ...
    for country, country_relations in grouped.items():
        country_level = [r for r in country_relations if r.location.name.casefold() == country.casefold()]
        province_relations = [r for r in country_relations if r.location.is_province]
        mentioned_provinces = [
            item.name for item in mentioned_locations
            if item.country.casefold() == country.casefold() and item.is_province
        ]
        selected_case = max((r.cases for r in country_level), default=0)
        if not country_level:
            selected_case = sum(r.cases for r in province_relations)
        deaths = sum((r.deaths or 0) for r in country_relations) or None
        frames = [r.time_frame for r in country_relations if r.time_frame]
        output_locations.append(SurveillanceLocation(
            country=country,
            provinces=list(dict.fromkeys([*mentioned_provinces, *(r.location.name for r in province_relations)])),
            reported_cases=selected_case,
            deaths=deaths,
            time_frame=frames[0] if frames else "",
        ))
